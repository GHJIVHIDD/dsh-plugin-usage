...
`;
		...
		function PriceForm(props) {
			...
			return el('div', { className: 'du-editForm' },
				...
				el('div', { className: 'du-editGrid' },
					...
					field('gIn', 'Plan input'),
					field('gCache', 'Plan cache'),
					field('gOut', 'Plan output'),
		...
		function UsageView(props) {
			...
			function startEdit(row) {
				...
				setEditing({
					...
					form: {
						...
						gIn: g.in != null ? String(g.in) : '',
						gCache: g.cache != null ? String(g.cache) : '',
						gOut: g.out != null ? String(g.out) : '',
			...
			function startAdd() {
				setErr(null);
				setEditing(null);
				setAddForm({ model: '', oPIn: '', oPCache: '', oPOut: '', oOIn: '', oOCache: '', oOOut: '', gIn: '', gCache: '', gOut: '', gQuota: '' });
				setAdding(true);
			}
			...
			const priceRows = table.map(function (row) {
				return el('div', { className: 'du-prow', key: row.key },
					...
					el('div', { className: 'du-pcell' }, row.go ? ('$' + row.go.in + '/' + row.go.cache + '/' + row.go.out + ' · quota $' + row.go.quota) : '—'),
