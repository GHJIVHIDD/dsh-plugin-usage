...
`;
		...
		// Format an ISO timestamp as local "MM-DD HH:MM".
		function fmtISO(iso) {
			if (!iso) return '—';
		...
		function UsageView(props) {
			...
			return el('div', { className: 'du-root' },
				...
				el('div', { className: 'du-scroll' },
					...
					el('div', { className: 'du-section' },
						el('div', { className: 'du-sectionHead' },
							el('span', null, 'OpenCode Go official quota'),
							el('span', { className: 'du-muted' }, oq.cachedAt ? 'sync ' + fmtTime(oq.cachedAt) : '')
						),
						oq.status === 'missing-key'
							? el('div', { className: 'du-empty' },
								'OpenCode Go API key not configured — set the DSH_OPENCODE_GO_KEY environment variable (the plugin also picks up OPENCODE_GO_API_KEY and DEEPSEEK_API_KEY automatically) and restart dsh.')
							: oq.status === 'error'
								? el('div', { className: 'du-empty' },
									'Failed to fetch official quota' + (oq.error ? ' (' + oq.error + ')' : '') + '.')
								: el('div', { className: 'du-bars' },
									(oq.windows || []).map(function (w) {
										const label = w.name === 'rolling' ? 'Rolling (5h)' : w.name === 'weekly' ? 'Weekly' : 'Monthly';
										const display = (w.percent != null ? w.percent + '%' : '—')
											+ (w.resetsAt ? ' · reset ' + fmtISO(w.resetsAt) : '');
										return Bar(label, display, w.percent != null ? w.percent : 0);
									})
								)
					),
					el('div', { className: 'du-section' },
