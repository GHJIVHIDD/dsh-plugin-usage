 ...
 *   - fetches the OpenCode Go official account quota from the official
 *     `opencode.ai/zen/go/v1/usage` API using the same credential the DSH
 *     providers already resolve (env → file → user-env), so no extra
 ...
 *   - The quota API key is resolved through the DSH credentials service for
 *     the request header only and is never persisted or printed.
...
// The API key is resolved exactly like the DSH providers do: the dedicated
// DSH_OPENCODE_GO_KEY env first, then the DSH credentials service (same
// resolution chain as llm-pi-ai / deepseek — env, file, user-env), probing the
// ref names used by the opencode-go provider config and the deepseek provider.
// The value is used for the request header only and is never stored or logged.
async function opencodeKey(ctx) {
  const env = (process && process.env) || {}
  if (env.DSH_OPENCODE_GO_KEY) return env.DSH_OPENCODE_GO_KEY
  try {
    const creds = ctx.get('credentials')
    if (creds) {
      for (const refName of ['OPENCODE_GO_API_KEY', 'DEEPSEEK_API_KEY']) {
        const resolved = await creds.resolve(credentialRef(refName))
    ...
    // credentials service unavailable — fall through to raw env
  }
  return env.OPENCODE_GO_API_KEY || env.DEEPSEEK_API_KEY || ''
}
...
function apply(ctx) {
  ...
  // Note: the stream wrapper calls finishLive() (status -> 'done') as soon as
  // the stream ends, and the assistant/message commit arrives *after* that, so
  // we must match ANY live record — not only 'running' ones — or every call
  // would be double-counted (once by llm/stream, once by addCall).
  function finalizeLive(sid, event) {
    ...
    for (let i = b.calls.length - 1; i >= 0; i--) {
      ...
      if (c.live && c.model === model && t >= end && t - end < 15000) {
        if (data.usage) {
  ...
  if (webServer) {
    ...
    route('/usage-api/state', async (req, res) => {
      ...
      respond(res, {
        ...
        totals: a.totals,
        quotas: a.quotas,
        officialQuota: await fetchOfficialQuota(ctx),
