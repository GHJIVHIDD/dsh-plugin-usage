...
// written to disk. Candidates, in order:
//   1. DSH_OPENCODE_GO_KEY    — dedicated key for this plugin (recommended)
//   2. OPENCODE_GO_API_KEY    — the env name used by the llm-pi-ai
//                               "opencode-go" provider configuration
//   3. DEEPSEEK_API_KEY       — default credential env of the deepseek
//                               provider, when that provider points at the
//                               OpenCode Go gateway
function opencodeKey() {
  const env = (process && process.env) || {}
  return env.DSH_OPENCODE_GO_KEY || env.OPENCODE_GO_API_KEY || env.DEEPSEEK_API_KEY || ''
}
