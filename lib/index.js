...
const GO = {
  'deepseek-v4-pro': { in: 0.435, out: 0.87, cache: 0.003625, quota: 15 },
  'deepseek-v4-flash': { in: 0.14, out: 0.28, cache: 0.0028, quota: 60 },
...
function apply(ctx) {
  ...
  function priceOf(model, usage, t) {
    ...
    const peak = off ? isPeakAt(t) : false
    const input = num(usage.inputTokens)
    const cacheRead = num(usage.cacheReadTokens)
    const cacheWrite = num(usage.cacheWriteTokens)
    const output = num(usage.outputTokens)
    const goCost = go
      ? (input * num(go.in) + cacheRead * num(go.cache) + cacheWrite * num(go.cacheWrite != null ? go.cacheWrite : go.cache) + output * num(go.out)) / 1e6
      : null
    const offCost = off
  ...
  function aggregate(sid) {
    ...
    const totals = {
      input: 0, output: 0, cacheRead: 0, cacheWrite: 0, reasoning: 0,
      calls: b.calls.length, running: 0, goCost: 0, offCost: 0, goQuota: null,
    }
    let lastModel = null
    ...
    for (const c of b.calls) {
      ...
      if (typeof p.go === 'number' && Number.isFinite(p.go)) totals.goCost += p.go
      if (typeof p.off === 'number' && Number.isFinite(p.off)) totals.offCost += p.off
      if (typeof p.goQuota === 'number') totals.goQuota = p.goQuota
      if (c.t >= lastT) {
        lastT = c.t
        lastModel = c.model
    ...
    return { totals, lastModel, lastProvider }
  }
  ...
  if (webServer) {
    ...
    route('/usage-api/prices', async (req, res) => {
      ...
      for (const k of keys) {
        const g = goPrice(k)
        if (g) go[k] = { in: g.in, out: g.out, cache: g.cache, cacheWrite: g.cacheWrite == null ? null : g.cacheWrite, quota: g.quota }
        const o = offPrice(k)
      ...
      for (const k of Object.keys(custom)) {
        ...
        customOut[k] = {
          ...
          go: c.go ? { in: c.go.in, out: c.go.out, cache: c.go.cache, cacheWrite: c.go.cacheWrite == null ? null : c.go.cacheWrite, quota: c.go.quota } : null,
        }
      }
